...
Вкладка «Карты» → «Дурак онлайн». Игра на двоих прямо в боте. Состояние партии целиком на сервере: клиент
получает только свои карты и количество карт соперника, а каждый ход
проверяется движком — сходить не по правилам или подсмотреть чужую руку
нельзя. Таблица `durak_rooms` приватная, анон-ключ к ней доступа не имеет.

**Приглашения.** Хозяин создаёт комнату и жмёт «Пригласить» — уходит ссылка
`t.me/<бот>?start=dk_КОД`. Соперник жмёт её, бот отвечает кнопкой, она
открывает мини-апп сразу за столом. Можно и просто продиктовать код —
в лобби есть поле «Войти по коду».
